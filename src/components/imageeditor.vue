<template>
  <div class="flex flex-col lg:flex-row max-w-[60rem] mx-auto ">
      <!-- Left Section (Image and Buttons) -->
      <div class="w-full lg:w-1/2 max-w-md mx-auto flex-grow bg-white shadow-2xl">
        ...
        <div class="absolute inset-0 flex items-center justify-center overflow-hidden rounded-full">
          ...
          >
            ...
            <img
              ...
              :src="appStore.processedImageUrl ?? appStore.imageUrl ?? undefined"
              alt="Profile"
  ...
  function downloadImage() {
    if (!appStore.processedImageUrl && !appStore.imageUrl) {
      console.warn("No image available to download");
      return;
    }
    const imageUrl = appStore.processedImageUrl ?? appStore.imageUrl;
    const link = document.createElement('a');
    link.href = imageUrl as string;
    link.download = 'headshot.png'; // Provide a filename for the download
